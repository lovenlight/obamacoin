...
#if defined(NDEBUG)
# error "Obamacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa84a822e63c4c469ef48591911e4af1e03a32b5b98f27acf548304df74179c48");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Obamacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Obamacoin Signed Message:\n";
...
{
    // Obamacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 44 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 300000); // Obamacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Obamacoin: 1 day
static const int64 nTargetSpacing = 2 * 60; // Obamacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Obamacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Obamacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5fc59535b5fd3e3ba0b8f5520a57e731b2df0cfc20db0cfe391fb5757ea9b4f6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Jun/2017 Fishin' Bait, World Renowned Mechanic, Dies at 67";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 44 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04efaed5b2b8a9493af0c9d85a23cea14d762c1b1c43462c8704cc481a18a16986ad3a3cb733b065ba4b510c0b65bb602be250acb4624d8940ba1848d223a243a5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515622928;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084664128;

        if (fTestNet)
        {
            block.nTime    = 1515622774;
            block.nNonce   = 386641665;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd9f5a10870952fb5edd6ed171e16cacaaebebfd0cb97f00dad01f9ac7136c1cd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc6, 0xbc, 0xd3 }; // Obamacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ObamacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ObamacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ObamacoinMiner(CWallet *pwallet)
{
    printf("ObamacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("obamacoin-miner");
    ...
    try { loop {
        ...
        printf("Running ObamacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ObamacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ObamacoinMiner, pwallet));
}
